...
4. FULL JOIN (or FULL OUTER JOIN): Returns all records when there is a match in either left or right table. If there is no match,
...
Goal: Retrieve a list of all employees and their departments, including employees with no department and departments with no
employees. 
SELECT
  e.employee_name,
  d.department_name
...
Result of the query
The output will show all employees and all departments, combining the matched rows and including unmatched rows with NULL values. 
...
103	        Peter Jones	101
104	        Alice Chen	102
Note: The ManagerID column refers to the EmployeeID of that employee's' manager. A NULL value indicates that the employee 
(in this case, John Smith) has no manager and is at the top of the hierarchy.
...
7. NATURAL JOIN: A natural join is based on all columns in the two tables that have the same name and selects rows with 
